...
import io.github.palexdev.mfxresources.fonts.MFXFontIcon;
 ...
 public class MainContoller implements Initializable {
     ...
     public MainContoller(Stage stage) {
         this.stage = stage;
     ...
     public void initialize(URL location, ResourceBundle resources) {
         ...
         Image image = new Image(ResourceLoader.load("Images/logo.png"), 64, 64, true, true);
         ImageView logo = new ImageView(image);
         Circle clip = new Circle(64);
     ...
     private void initializeLoader() {
        ...
        //  loader.addView(MFXLoaderBean.of("BUTTONS", loadURL("fxml/Buttons.fxml")).setBeanToNodeMapper(() -> createToggle("fas-circle-dot")).setDefaultRoot(true).get());
         ...
         loader.addView(MFXLoaderBean.of("FRIENDS", loadURL("fxml/Friend.fxml")).setBeanToNodeMapper(() -> createToggle("fas-toggle-on")).setControllerFactory(c -> new FriendController(stage)).get());
         loader.addView(MFXLoaderBean.of("NOTIFICATIONS", loadURL("fxml/Notification.fxml")).setBeanToNodeMapper(() -> createToggle("fas-bell")).setControllerFactory(c -> new NotificationContoller(stage)).get());
         loader.addView(MFXLoaderBean.of("ROOMS", loadURL("fxml/Rooms.fxml")).setBeanToNodeMapper(() -> createToggle("fas-square-caret-down")).get());
         loader.addView(MFXLoaderBean.of("CHANNEL", loadURL("fxml/Channel.fxml")).setBeanToNodeMapper(() -> createToggle("fas-toggle-on")).get());
        //  loader.addView(MFXLoaderBean.of("CHANNELFRIEND", loadURL("fxml/ChannelFreind.fxml")).get());
     ...
     private ToggleButton createToggle(String icon) {
         return createToggle(icon, 0);
     }
 
     private ToggleButton createToggle(String icon, double rotate) {
         MFXFontIcon wrapper = new MFXFontIcon(icon, 24);
         MFXRectangleToggleNode toggleNode = new MFXRectangleToggleNode();
         toggleNode.setAlignment(Pos.CENTER);
         toggleNode.getStyleClass().add("icon-toggle-button");
         toggleNode.setMaxWidth(Double.MAX_VALUE);
         toggleNode.setToggleGroup(toggleGroup);
         toggleNode.setGraphic(wrapper);
         if (rotate != 0) wrapper.setRotate(rotate);
         return toggleNode;
     }
