...
public class ConnectedDAO extends DAO<Connected> {
  ...
  public void createFriendUser(UserInformation user,String rommename){
    String uuid_room;
    try {
      PreparedStatement ps = this.connect.prepareStatement("SELECT uuid_room FROM room WHERE roomname = ?");
      ps.setString(1, rommename);
      ResultSet rs = ps.executeQuery();
      if (rs.next()) {
        uuid_room = rs.getString("uuid_room");
      } else {
    ...
    try {
      ...
      statement.setString(2, uuid_room);
